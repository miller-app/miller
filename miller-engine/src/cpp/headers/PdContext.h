...
/**
 * The <code>PdContext</code> is a container for a set of <code>PdGraph</code>s
 * operating in a common environment. The <code>PdContext</code> tracks all
 * relevent global variables.
 */
class PdContext {

  public:
    PdContext(int numInputChannels, int numOutputChannels, int blockSize,
              float sampleRate,
    ...
    /**
     * Attach the given <code>graph</code> to this <code>context</code>, also
     * registering all necessary objects, and computing the dsp object compute
     * order if necessary.
     */
    ...
    /** Globally register a [receive~] object. Connect to registered [send~]
     * objects with the same name. */
    void registerDspReceive(DspReceive *dspReceive);
    void unregisterDspReceive(DspReceive *dspReceive);

    /** Globally register a [send~] object. Connect to registered [receive~]
     * objects with the same name. */
    ...
    /** Updates the input buffers all attached objects to the DspReceives
     * associated with the given DspSend. */
    void updateDspReceiveForSendWithBuffer(const char *name, float *buffer);

    /**
     * Globally register a [delwrite~] object. Registration is necessary such
     * that they can be connected to [delread~] and [vd~] objects as are they
    ...
    /**
     * Globally register a [delread~] or [vd~] object. Registration is necessary
     * such that they can be connected to [delwrite~] objects are they are added
     * to the graph.
     */
    ...
    /**
     * Schedules a message to be sent to all receivers at the start of the next
     * block.
     * @returns The <code>PdMessage</code> which will be sent. It is intended
     * that the programmer will set the values of the message with a call to
    ...
    /**
     * Schedules a message described by the given string to be sent to named
     * receivers at the given timestamp.
     */
     ...
     * specified <code>time</code>. The message will be copied to the heap and
     * the context will thereafter take over ownership and be responsible for
     * freeing it. The pointer to the heap-message is returned.
     */
    PdMessage *scheduleMessage(MessageObject *messageObject,
                               unsigned int outletIndex, PdMessage *message);

    /**
     * Cancel a scheduled <code>PdMessage</code> according to its id. The
     * message memory will be freed.
     */
    void cancelMessage(MessageObject *messageObject, int outletIndex,
                       PdMessage *message);

    /** Receives and processes messages sent to the Pd system by sending to
     * "pd". */
    ...
    MessageObject *newObject(const char *objectLabel, PdMessage *initMessage,
                             PdGraph *graph);
    ...
    void registerExternalObject(const char *objectLabel,
                                MessageObject *(*newObject)(PdMessage *,
                                                            PdGraph *));
  ...
  private:
    /** Returns <code>true</code> if the graph was successfully configured.
     * <code>false</code> otherwise. */
    bool configureEmptyGraphWithParser(PdGraph *graph,
                                       PdFileParser *fileParser);
