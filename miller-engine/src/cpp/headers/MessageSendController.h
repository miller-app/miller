...
/**
 * Because of features such as external message injection and implicit message
 * sending from message boxes, it must be possible to [send] a message to
 * associated [receive]ers without explicitly requiring a [send] object to exist
 * in the graph. The <code>MessageSendController</code> is intended as a central
 * dispatcher for all sent messages, delivering a message to all associated
 * [receive]ers without requiring the existance of a [send] object.
 *
 * It can deliver messages via the usual <code>receiveMessage()</code> function,
 * where the inlet index identifies a receiver name. The index for a receiver
 * name can be determined with <code>getNameIndex()</code>. This is useful for
 * messages that must be scheduled in the global message queue and must then be
 * dispatched via the <code>receiveMessage()</code> function.
 *
 * Alternatively, a message can be sent to receivers using
 * <code>receiveMessage()</code> with name and message arguments (instead of
 * inlet index and message). Messages sent using this alternative will be sent
 * right away (avoiding the message queue).
 */
class MessageSendController : public MessageObject {
  ...
  public:
    ...
    /** Returns true if a receiver by that name is already registered. False
     * otherwise. */
