...
class BufferPool {
  public:
    ...
    /**
     * If a buffer is available, it is reserved with a number of dependencies.
     * Otherwise a new buffer is created.
     */
    float *getBuffer(unsigned int numDependencies);

    /** The reference count of the buffer is reduced by one, possibly made
    ...
    unsigned int getNumTotalBuffers() {
        return (pool.size() + reserved.size());
    }

  private:
    /** A pool of reserved buffers, paired with the number of outstanding
     * dependencies. */
