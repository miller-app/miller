...
MessageObject *MessageListAppend::newObject(PdMessage *initMessage,
                                            PdGraph *graph) {
    if (initMessage->isSymbol(0)) {
        if (initMessage->isSymbol(0, "append") ||
            ...
            memcpy(message->getElement(0), initMessage->getElement(1),
...
MessageListAppend::MessageListAppend(PdMessage *initMessage, PdGraph *graph)
    : MessageObject(2, 1, graph) {
    appendMessage = initMessage->copyToHeap();
}

MessageListAppend::~MessageListAppend() { appendMessage->freeMessage(); }

bool MessageListAppend::shouldDistributeMessageToInlets() { return false; }

void MessageListAppend::processMessage(int inletIndex, PdMessage *message) {
    switch (inletIndex) {
    case 0: {
        // if the incoming message is a bang, then it is considered to be a list
        // of length zero
        int numMessageElements =
            (!message->isBang(0)) ? message->getNumElements() : 0;
        ...
        if (numTotalElements > 0) {
            ...
            outgoingMessage->initWithTimestampAndNumElements(
                message->getTimestamp(), numTotalElements);
            memcpy(outgoingMessage->getElement(0), message->getElement(0),
                   numMessageElements * sizeof(MessageAtom));
            memcpy(outgoingMessage->getElement(numMessageElements),
                   appendMessage->getElement(0),
                   numAppendElements * sizeof(MessageAtom));
