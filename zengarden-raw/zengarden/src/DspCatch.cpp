...
DspCatch::DspCatch(PdMessage *initMessage, PdGraph *graph)
    : DspObject(0, 0, 0, 1, graph) {
...
void DspCatch::addThrow(DspThrow *dspThrow) {
    if (!strcmp(
            dspThrow->getName(),
            name)) { // make sure that the throw~ really does match this catch~
        ...
        case 0:
            ...
            break;
        case 1:
            processFunction = &processOne;
            break;
        default:
            processFunction = &processMany;
            break;
        }
    }
}
...
void DspCatch::processOne(DspObject *dspObject, int fromIndex, int toIndex) {
    ...
    memcpy(d->dspBufferAtOutlet[0], dspThrow->getBuffer(),
...
void DspCatch::processMany(DspObject *dspObject, int fromIndex, int toIndex) {
    ...
    ArrayArithmetic::add((*it++)->getBuffer(), (*it++)->getBuffer(),
                         d->dspBufferAtOutlet[0], 0, toIndex);
    while (it != d->throwList.end()) {
        ArrayArithmetic::add(d->dspBufferAtOutlet[0], (*it++)->getBuffer(),
                             d->dspBufferAtOutlet[0], 0, toIndex);
    };
}

// catch objects should be processed after their corresponding throw object even
    ...
    if (isOrdered) {
        ...
        for (int i = 0; i < getNumDspOutlets(); i++) {
            if (canSetBufferAtOutlet(i)) {
                float *buffer = graph->getContext()->getBufferPool()->getBuffer(
                    outgoingDspConnections[i].size());
