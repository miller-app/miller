...
DeclareList::DeclareList() { declareList = list<string>(); }
...
bool DeclareList::isFullPath(const char *path) { return (path[0] == '/'); }

bool DeclareList::hasTrailingSlash(const char *path) {
    return (path[strlen(path) - 1] == '/');
}

char *DeclareList::getRootPath() { return (char *)declareList.front().c_str(); }

list<string>::iterator DeclareList::getIterator() {
    return declareList.begin();
}

list<string>::iterator DeclareList::getEnd() { return declareList.end(); }
