...
DspTablePlay::DspTablePlay(PdMessage *initMessage, PdGraph *graph)
    : DspObject(1, 0, 2, 1, graph) {
...
DspTablePlay::~DspTablePlay() { free(name); }

void DspTablePlay::setTable(MessageTable *aTable) { table = aTable; }

ConnectionType DspTablePlay::getConnectionType(int outletIndex) {
    // the right outlet is a message outlet, otherwise this object is considered
    // to output audio
...
void DspTablePlay::playTable(int startIndex, int duration, double startTime) {
    if (startIndex >= 0 && duration >= -1) {
        if (outgoingMessage != NULL) {
            // if the table is currently playing, i.e. there is an outstanding
            // scheduled message, cancel it
        ...
        if (startIndex < bufferLength) {
            // sanity check that table should be played from a point before it
            // ends
            currentTableIndex = startIndex;
            endTableIndex =
                (duration == -1) ? bufferLength : startIndex + duration;
            if (endTableIndex > bufferLength) {
                endTableIndex = bufferLength;
            }
            double durationMs = 1000.0 *
                                ((double)(endTableIndex - startIndex)) /
                                (double)graph->getSampleRate();
...
void DspTablePlay::processDspWithIndex(int fromIndex, int toIndex) {
    if (table != NULL) {
        ...
        if (bufferLength < endTableIndex) {
            // in case the table length has been reset while tabplay~ is playing
            // the buffer
            endTableIndex = bufferLength;
        }
        int duration =
            toIndex - fromIndex; // the duration of the output buffer to fill
        ...
        if (remainingTableSamples <= 0) {
            // if the entire buffer has already been read, fill the output with
            // silence
            memset(dspBufferAtOutlet[0], 0, blockSizeInt * sizeof(float));
        } else if (duration <= remainingTableSamples) {
            if (duration == blockSizeInt) {
                // if the entire output must be filled and there are more than
                // one buffer's worth of samples still available from the table,
                // just set the output buffer pointer
                memcpy(dspBufferAtOutlet[0], tableBuffer + currentTableIndex,
                       blockSizeInt * sizeof(float));
                currentTableIndex += blockSizeInt;
            } else {
                // if the number of remaining samples in the table is more than
                // the number of samples which need to be read to the output
                // buffer, but not the whole output buffer must be written
                memcpy(dspBufferAtOutlet[0] + fromIndex,
                       tableBuffer + currentTableIndex,
                       duration * sizeof(float));
                currentTableIndex += duration;
            }
        } else {
            // if the number of output buffer samples to fill is larger than the
            // number of remaining table samples, fill the output with the
            // maximum available table samples, and fill in the remainder with
            // zero
            memcpy(dspBufferAtOutlet[0] + fromIndex,
                   tableBuffer + currentTableIndex,
