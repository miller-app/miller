...
MessageObject *DspBandpassFilter::newObject(PdMessage *initMessage,
                                            PdGraph *graph) {
    return new DspBandpassFilter(initMessage, graph);
}

DspBandpassFilter::DspBandpassFilter(PdMessage *initMessage, PdGraph *graph)
    : DspFilter(3, graph) {
    fc = initMessage->isFloat(0) ? initMessage->getFloat(0)
                                 : graph->getSampleRate() / 2.0f;
...
void DspBandpassFilter::processMessage(int inletIndex, PdMessage *message) {
    ...
    case 1: {
        if (message->isFloat(0))
            calcFiltCoeff(message->getFloat(0), q);
        break;
    }
    case 2: {
        if (message->isFloat(0))
            calcFiltCoeff(fc, message->getFloat(0));
        break;
    }
    default:
        break;
