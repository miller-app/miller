...
MessageObject *MessageUnpack::newObject(PdMessage *initMessage,
                                        PdGraph *graph) {
...
MessageUnpack::MessageUnpack(PdMessage *initMessage, PdGraph *graph)
    : MessageObject(1,
                    (initMessage->getNumElements() < 2)
                        ? 2
                        : initMessage->getNumElements(),
                    graph) {
    if (initMessage->getNumElements() < 2) {
        // if unpack is not initialised with anything, assume two "anything"
        // outputs
...
MessageUnpack::~MessageUnpack() { templateMessage->freeMessage(); }
...
void MessageUnpack::processMessage(int inletIndex, PdMessage *message) {
    ...
    for (int i = numElements - 1; i >= 0; i--) {
        ...
        if (elementType == message->getType(i) || elementType == ANYTHING) {
            ...
            graph->printErr(
                "unpack: type mismatch: %s expected but got %s.",
