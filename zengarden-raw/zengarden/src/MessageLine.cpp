...
MessageLine::MessageLine(PdMessage *initMessage, PdGraph *graph)
    : MessageObject(2, 1, graph) {
    currentValue = initMessage->isFloat(0) ? initMessage->getFloat(0) : 0.0f;
    grainRate = initMessage->isFloat(1) ? (double)initMessage->getFloat(1)
                                        : DEFAULT_GRAIN_RATE;
    slope = 0.0f;
    pendingMessage = NULL;
...
void MessageLine::processMessage(int inletIndex, PdMessage *message) {
        ...
        case 1: {
            if (message->isFloat(0)) {
                ...
                outgoingMessage->initWithTimestampAndFloat(
                    message->getTimestamp(), currentValue);
        ...
        case 2: {
            if (message->hasFormat("ff")) {
                ...
                if (pendingMessage != NULL) {
                    ...
                    // calculate the new current value depending on when the
                    // last message as sent from this object
                    currentValue +=
                        (message->getTimestamp() - lastMessageTimestamp) *
                        slope;
                }
                slope = (targetValue - currentValue) / duration;

                // cancel any previous pending messages. The next message will
                // be scheduled in sendMessage()
                cancelPendingMessage();
                ...
                // set the current value to the given input, without outputting
                // any message
    ...
    case 1: {
        ...
        if (message->isFloat(0)) {
            // update the grain rate, because somehow that makes sense. Could be
            // completely wrong :-/
...
void MessageLine::sendMessage(int outletIndex, PdMessage *message) {
    // now that this message is being sent, the current value of this [line]
    // object is certain
    currentValue = message->getFloat(0);
    if (slope > 0.0f) {
        if (currentValue < targetValue) {
            pendingMessage = PD_MESSAGE_ON_STACK(1);
            pendingMessage->initWithTimestampAndFloat(
                message->getTimestamp() + grainRate,
                currentValue + slope * grainRate);
            pendingMessage = graph->scheduleMessage(this, 0, pendingMessage);
        } else { // currentValue >= targetValue
            // in case the current value is greater than the target value, due
