...
MessagePack::~MessagePack() { outgoingMessage->freeMessage(); }
...
void MessagePack::processMessage(int inletIndex, PdMessage *message) {
    ...
    case FLOAT: {
        if (outgoingMessage->isFloat(inletIndex)) {
            ...
            graph->printErr(
                "pack: type mismatch: %s expected but got %s at inlet %i.\n",
                StaticUtils::messageElementTypeToString(
                    outgoingMessage->getType(inletIndex)),
    ...
    case SYMBOL: {
        if (outgoingMessage->isSymbol(inletIndex)) {
            // NOTE(mhroth): this approach can lead to a lot of fragemented
            // memory if symbols are replaced often
            free(outgoingMessage->getSymbol(
                inletIndex)); // free the preexisting symbol on the heap
            // create a new symbol on the heap and store it in the outgoing
            // message
            outgoingMessage->setSymbol(
                inletIndex, StaticUtils::copyString(message->getSymbol(0)));
            onBangAtInlet(inletIndex, message->getTimestamp());
        } else {
            graph->printErr(
                "pack: type mismatch: %s expected but got %s at inlet %i.\n",
