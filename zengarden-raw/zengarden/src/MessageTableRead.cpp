...
MessageObject *MessageTableRead::newObject(PdMessage *initMessage,
                                           PdGraph *graph) {
    return new MessageTableRead(initMessage, graph);
}

MessageTableRead::MessageTableRead(PdMessage *initMessage, PdGraph *graph)
...
MessageTableRead::~MessageTableRead() { free(name); }

char *MessageTableRead::getName() { return name; }

void MessageTableRead::setTable(MessageTable *aTable) { table = aTable; }
        ...
        if (table != NULL) {
            ...
            if (index >= 0 && index < bufferLength) {
                ...
                outgoingMessage->initWithTimestampAndFloat(
                    message->getTimestamp(), buffer[index]);
