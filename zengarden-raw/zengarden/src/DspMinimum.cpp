...
DspMinimum::DspMinimum(PdMessage *initMessage, PdGraph *graph)
...
void DspMinimum::onInletConnectionUpdate(unsigned int inletIndex) {
    processFunction = (!incomingDspConnections[0].empty() &&
                       !incomingDspConnections[1].empty())
                          ? &processSignal
                          : &processScalar;
}

void DspMinimum::processSignal(DspObject *dspObject, int fromIndex,
                               int toIndex) {
    ...
    while (toIndex) {
        _mm_store_ps(output,
                     _mm_min_ps(_mm_load_ps(input0), _mm_load_ps(input1)));
        input0 += 4;
        input1 += 4;
        output += 4;
        toIndex -= 4;
...
void DspMinimum::processScalar(DspObject *dspObject, int fromIndex,
                               int toIndex) {
    DspMinimum *d = reinterpret_cast<DspMinimum *>(dspObject);
#if __APPLE__
    ...
    vDSP_vmin(d->dspBufferAtInlet[0] + fromIndex, 1, vconst, 1,
              d->dspBufferAtOutlet[0] + fromIndex, 1, duration);
#else
