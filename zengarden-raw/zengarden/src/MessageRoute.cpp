...
MessageRoute::~MessageRoute() { routeMessage->freeMessage(); }

void MessageRoute::processMessage(int inletIndex, PdMessage *message) {
    int numRouteChecks = routeMessage->getNumElements();
    int outletIndex =
        numRouteChecks; // by default, send the message out of the right outlet
    ...
    if (outletIndex == numRouteChecks) {
        ...
        memcpy(outgoingMessage->getElement(0), message->getElement(1),
