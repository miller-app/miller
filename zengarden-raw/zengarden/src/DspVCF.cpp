...
DspVCF::DspVCF(PdMessage *initMessage, PdGraph *graph)
    : DspObject(3, 3, 0, 2, graph) {
    sampleRate = graph->getSampleRate();
    calculateFilterCoefficients(this->sampleRate / 2.0f,
                                1.0f); // initialise the filter completely open
    tap_0 = tap_1 = 0.0f;
}
...
const char *DspVCF::getObjectLabel() { return "vcf~"; }
...
void DspVCF::processDspWithIndex(int fromIndex, int toIndex) {
    ...
    for (int i = fromIndex; i < toIndex; i++) {
      ...
      dspBufferAtOutlet0[i] = dspBufferAtInlet0[i] + (coef1 * tap_0) + (coef2 *
    tap_1); tap_1 = tap_0; tap_0 = dspBufferAtOutlet0[i];
    }
    // dspBufferAtOutlet0[i] *= gain;
    ArrayArithmetic::multiply(dspBufferAtOutlet0, gain, dspBufferAtOutlet0,
    fromIndex, toIndex);
    */
    // NOTE(mhroth): This object is definitely not working properly. It outputs
    // only zero.
