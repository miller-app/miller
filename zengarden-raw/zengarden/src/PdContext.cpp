...
PdContext::PdContext(int numInputChannels, int numOutputChannels, int blockSize,
                     float sampleRate,
                     void *(*function)(ZGCallbackFunction, void *, void *),
                     void *userData) {
    ...
    globalDspInputBuffers = (numBytesInInputBuffers > 0)
                                ? ALLOC_ALIGNED_BUFFER(numBytesInInputBuffers)
                                : NULL;
    memset(globalDspInputBuffers, 0, numBytesInInputBuffers);
    globalDspOutputBuffers = (numBytesInOutputBuffers > 0)
                                 ? ALLOC_ALIGNED_BUFFER(numBytesInOutputBuffers)
                                 : NULL;
...
void PdContext::registerExternalObject(
...
int PdContext::getNumInputChannels() { return numInputChannels; }

int PdContext::getNumOutputChannels() { return numOutputChannels; }

int PdContext::getBlockSize() { return blockSize; }

float PdContext::getSampleRate() { return sampleRate; }
...
double PdContext::getBlockStartTimestamp() { return blockStartTimestamp; }

double PdContext::getBlockDuration() { return blockDurationMs; }

int PdContext::getNextGraphId() { return ++globalGraphId; }
...
void PdContext::process(float *inputBuffers, float *outputBuffers) {
    ...
    while (
        !messageCallbackQueue->empty() &&
        (omlPair = messageCallbackQueue->peek()).second.first->getTimestamp() <
            nextBlockStartTimestamp) {
        ...
        if (message->getTimestamp() < blockStartTimestamp) {
            // messages injected into the system with a timestamp behind the
            // current block are automatically rescheduled for the beginning of
            // the current block. This is done in order to normalise the
            // treament of messages, but also to avoid difficulties in cases
            // when messages are scheduled in subgraphs with different block
            // sizes.
            message->setTimestamp(blockStartTimestamp);
        }

        object->sendMessage(outletIndex, message);
        message->freeMessage(); // free the message now that it has been sent
                                // and processed
    }
    ...
    case 0:
        break;
    case 1:
        graphList.front()->processFunction(graphList.front(), 0, 0);
        break;
    default: {
...
MessageObject *PdContext::newObject(const char *objectLabel,
                                    PdMessage *initMessage, PdGraph *graph) {
    MessageObject *messageObject =
        objectFactoryMap->newObject(objectLabel, initMessage, graph);
...
void PdContext::registerDspReceive(DspReceive *dspReceive) {
    ...
    list<DspReceive *> *receiveList =
        &(dspReceiveMap[string(dspReceive->getName())]);
...
void PdContext::unregisterDspReceive(DspReceive *dspReceive) {
    ...
    dspReceive->setDspBufferAtInlet(
        dspReceive->getGraph()->getBufferPool()->getZeroBuffer(), 0);
}

void PdContext::registerDspSend(DspSend *dspSend) {
    DspSend *sendObject = getDspSend(dspSend->getName());
    if (sendObject != NULL) {
        printErr("Duplicate send~ object found with name \"%s\".",
                 dspSend->getName());
        return;
    }
    ...
    updateDspReceiveForSendWithBuffer(dspSend->getName(),
                                      dspSend->getDspBufferAtOutlet(0));
}

void PdContext::unregisterDspSend(DspSend *dspSend) {
    dspSendList.remove(dspSend);

    // inform all previously connected receive~s that the send~ buffer does not
    // exist anymore.
    updateDspReceiveForSendWithBuffer(
        dspSend->getName(),
        dspSend->getGraph()->getBufferPool()->getZeroBuffer());
}

DspSend *PdContext::getDspSend(const char *name) {
    for (list<DspSend *>::iterator it = dspSendList.begin();
         it != dspSendList.end(); ++it) {
...
void PdContext::updateDspReceiveForSendWithBuffer(const char *name,
                                                  float *buffer) {
    list<DspReceive *> receiveList = dspReceiveMap[string(name)];
    for (list<DspReceive *>::iterator it = receiveList.begin();
         it != receiveList.end(); ++it) {
...
void PdContext::unregisterRemoteMessageReceiver(
    RemoteMessageReceiver *receiver) {
...
void PdContext::registerDelayline(DspDelayWrite *delayline) {
    if (getDelayline(delayline->getName()) != NULL) {
        printErr("delwrite~ with duplicate name \"%s\" registered.",
                 delayline->getName());
        return;
    }
    ...
    for (list<DelayReceiver *>::iterator it = delayReceiverList.begin();
         it != delayReceiverList.end(); it++) {
        if (!strcmp((*it)->getName(), delayline->getName()))
            (*it)->setDelayline(delayline);
    }
}
...
DspDelayWrite *PdContext::getDelayline(const char *name) {
    for (list<DspDelayWrite *>::iterator it = delaylineList.begin();
         it != delaylineList.end(); ++it) {
        if (!strcmp((*it)->getName(), name))
            return *it;
    }
    return NULL;
...
void PdContext::registerDspCatch(DspCatch *dspCatch) {
    ...
    for (list<DspThrow *>::iterator it = throwList.begin();
         it != throwList.end(); it++) {
        if (!strcmp((*it)->getName(), dspCatch->getName()))
            dspCatch->addThrow((*it));
    }
}

DspCatch *PdContext::getDspCatch(const char *name) {
    for (list<DspCatch *>::iterator it = catchList.begin();
         it != catchList.end(); it++) {
        if (!strcmp((*it)->getName(), name))
            return (*it);
    }
...
void PdContext::registerTable(MessageTable *table) {
    ...
    for (list<TableReceiverInterface *>::iterator it =
             tableReceiverList.begin();
         it != tableReceiverList.end(); it++) {
        // in case the table receiver doesn't have the table name yet
        if ((*it)->getName() == NULL)
            continue;
        if (!strcmp((*it)->getName(), table->getName()))
            (*it)->setTable(table);
    }
}

MessageTable *PdContext::getTable(const char *name) {
    for (list<MessageTable *>::iterator it = tableList.begin();
         it != tableList.end(); it++) {
        if (!strcmp((*it)->getName(), name))
            return (*it);
    }
...
void PdContext::registerExternalReceiver(const char *receiverName) {
    lock(); // don't update the external receiver registry while processing it,
            // of course!
...
void PdContext::scheduleExternalMessageV(const char *receiverName,
                                         double timestamp,
...
void PdContext::scheduleExternalMessage(const char *receiverName,
                                        PdMessage *message) {
    lock();
...
void PdContext::scheduleExternalMessage(const char *receiverName,
    ...
    // do the heavy lifting of string parsing before the lock (minimise the
    // critical section)
...
PdMessage *PdContext::scheduleMessage(MessageObject *messageObject,
                                      unsigned int outletIndex,
                                      PdMessage *message) {
    // basic argument checking. It may happen that the message is NULL in case a
    // cancel message is sent multiple times to a particular object, when no
    // message is pending
    if (message != NULL && messageObject != NULL) {
        message = message->copyToHeap();
        messageCallbackQueue->insertMessage(messageObject, outletIndex,
                                            message);
        return message;
    }
    return NULL;
...
void PdContext::cancelMessage(MessageObject *messageObject, int outletIndex,
                              PdMessage *message) {
    if (message != NULL && outletIndex >= 0 && messageObject != NULL) {
        messageCallbackQueue->removeMessage(messageObject, outletIndex,
                                            message);
        message->freeMessage();
    }
}
