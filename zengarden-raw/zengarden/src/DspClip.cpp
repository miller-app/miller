...
DspClip::DspClip(PdMessage *initMessage, PdGraph *graph)
    : DspObject(3, 1, 0, 1, graph) {
...
std::string DspClip::toString() {
    char str[snprintf(NULL, 0, "%s %g %g", getObjectLabel(), lowerBound,
                      upperBound) +
             1];
    snprintf(str, sizeof(str), "%s %g %g", getObjectLabel(), lowerBound,
             upperBound);
    return str;
}
...
void DspClip::processMessage(int inletIndex, PdMessage *message) {
    ...
    case 1:
        ...
        break; // set the lower bound
    case 2:
        if (message->isFloat(0))
            upperBound = message->getFloat(0);
        break; // set the upper bound
    default:
        break;
    }
}
...
#if __APPLE__
    vDSP_vclip(d->dspBufferAtInlet[0] + fromIndex, 1, &(d->lowerBound),
               &(d->upperBound), d->dspBufferAtOutlet[0] + fromIndex, 1,
               toIndex - fromIndex);
#else
