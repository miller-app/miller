...
// MessageInlet is initialised with an inlet because it manages connections from
// outside of the containing graph.
...
ObjectType MessageInlet::getObjectType() { return MESSAGE_INLET; }
...
list<DspObject *> MessageInlet::getProcessOrder() {
    // a MessageInlet always returns an empty list as it does not process any
    // audio
...
list<DspObject *> MessageInlet::getProcessOrderFromInlet() {
    ...
    for (list<ObjectLetPair>::iterator it =
             incomingMessageConnections[0].begin();
