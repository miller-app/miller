...
DspLine::DspLine(PdMessage *initMessage, PdGraph *graph)
    : DspObject(2, 0, 0, 1, graph) {
    target = 0.0f;
    slope = 0.0f;
    ...
    if (inletIndex == 0) { // not sure what the right inlet is for
        ...
        default: { // at least two inputs
            ...
            if (message->isFloat(0) && message->isFloat(1)) {
                target = message->getFloat(0);
                float timeToTargetMs =
                    message->getFloat(1); // no negative time to targets!
                numSamplesToTarget = StaticUtils::millisecondsToSamples(
                    (timeToTargetMs < 1.0f) ? 1.0f : timeToTargetMs,
                    graph->getSampleRate());
...
void DspLine::processDspWithIndex(int fromIndex, int toIndex) {
    if (numSamplesToTarget <= 0.0f) { // if we have already reached the target
        ...
        if (n > 0) { // n may be zero
            ArrayArithmetic::fill(dspBufferAtOutlet[0], target, fromIndex,
                                  toIndex);
            lastOutputSample = target;
        }
    } else {
        ...
        int n = toIndex - fromIndex;
        if (n > 0) { // n may be zero
            // if there is anything to process at all (several messages may be
            // received at once)
            if (numSamplesToTarget < n) {
                int targetIndexInt = fromIndex + numSamplesToTarget;
#if __APPLE__
                vDSP_vramp(&lastOutputSample, &slope,
                           dspBufferAtOutlet[0] + fromIndex, 1,
                           targetIndexInt - fromIndex);
                vDSP_vfill(&target, dspBufferAtOutlet[0] + targetIndexInt, 1,
                           toIndex - targetIndexInt);
#else
                // if we will process more samples than we have remaining to the
                // target i.e., if we will arrive at the target while processing
                dspBufferAtOutlet[0][fromIndex] = lastOutputSample;
                for (int i = fromIndex + 1; i < targetIndexInt; i++) {
                    dspBufferAtOutlet[0][i] =
                        dspBufferAtOutlet[0][i - 1] + slope;
                }
...
#if __APPLE__
                vDSP_vramp(&lastOutputSample, &slope,
                           dspBufferAtOutlet[0] + fromIndex, 1, n);
#else
                dspBufferAtOutlet[0][fromIndex] = lastOutputSample;
