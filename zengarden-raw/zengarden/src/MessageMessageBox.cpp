...
MessageObject *MessageMessageBox::newObject(PdMessage *initString,
                                            PdGraph *graph) {
 ...
 * A) The simplest case is one where only one message is specified, including a
 * list of primitives which should be included in one message. The list may also
 * include variable indicies (in the form of, e.g. $1, $2, etc.) which refer to
 * those locations in the incoming message which triggers the message box. B) A
 * slightly more complicated case is where several messages in the form of A)
 * are separated by a comma (','). Each of the messages is processed and sent
 * independently from the message box when it is triggered. C) The most complex
 * case is where messages in the form of A) are separated by a semicolon (';').
 * The first symbol is the name of a message receiver. The remainder of the
 * string is converted into a message.
 */
MessageMessageBox::MessageMessageBox(char *initString, PdGraph *graph)
    ...
    vector<string> messageInitListAll =
        StaticUtils::tokenizeString(initString, "\\;");

    // parse the first "message" for individual messages that should be sent
    // from the outlet
    vector<string> messageInitList = StaticUtils::tokenizeString(
        (char *)messageInitListAll[0].c_str(), "\\,");
    for (int i = 0; i < messageInitList.size(); i++) {
        string initString = messageInitList[i];
        int maxElements = (initString.size() / 2) + 1;
        // NOTE(mhroth): though this is alloca is in a for loop, it is not
        // expected that the compiler will do anything funny, like unrolling the
        // loop, thereby causing unexpected stack overflows
        PdMessage *message = PD_MESSAGE_ON_STACK(maxElements);
        // StaticUtils::tokenizeString does not remove the trailing ";" from the
        // original string. We should not process it because it will result in
        // an empty message.
        if (strcmp(initString.c_str(), ";") != 0) {
            char str[initString.size() + 1];
    ...
    for (int i = 1; i < messageInitListAll.size(); i++) {
        ...
        if (strcmp(initString.c_str(), ";") != 0) {
            // trim any leading spaces. This will otherwise message up the
            // parsing
            ...
            PdMessage *message = PD_MESSAGE_ON_STACK(maxElements);
            char str[messageString.size() + 1];
            strcpy(str, messageString.c_str());
            message->initWithString(0.0, maxElements, str);
            MessageNamedDestination namedDestination = make_pair(
                StaticUtils::copyString(name.c_str()), message->copyToHeap());
...
#define RES_BUFFER_LENGTH 64
    char resolvedName[RES_BUFFER_LENGTH]; // resolution buffer for named
                                          // destination
    ...
    for (int i = 0; i < localMessageList.size(); i++) {
        ...
        outgoingMessage->initWithTimestampAndNumElements(
            message->getTimestamp(), numElements);
        memcpy(outgoingMessage->getElement(0), messageTemplate->getElement(0),
        ...
        for (int i = 0; i < numElements; i++) {
            if (messageTemplate->isSymbol(i)) {
                ...
                PdMessage::resolveString(messageTemplate->getSymbol(i), message,
                                         1, buffer, RES_BUFFER_LENGTH);
                outgoingMessage->parseAndSetMessageElement(
                    i, buffer); // buffer is resolved to float or string
            }
        }
    ...
    for (int i = 0; i < remoteMessageList.size(); i++) {
        ...
        PdMessage::resolveString(namedDestination.first, message, 1,
                                 resolvedName, RES_BUFFER_LENGTH);
