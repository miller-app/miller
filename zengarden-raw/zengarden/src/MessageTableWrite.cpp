...
MessageObject *MessageTableWrite::newObject(PdMessage *initMessage,
                                            PdGraph *graph) {
    return new MessageTableWrite(initMessage, graph);
}

MessageTableWrite::MessageTableWrite(PdMessage *initMessage, PdGraph *graph)
    : MessageObject(2, 0, graph) {
...
MessageTableWrite::~MessageTableWrite() { free(name); }

char *MessageTableWrite::getName() { return name; }

void MessageTableWrite::setTable(MessageTable *aTable) { table = aTable; }

bool MessageTableWrite::shouldDistributeMessageToInlets() { return false; }
