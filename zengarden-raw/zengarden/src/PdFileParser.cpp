...
PdFileParser::PdFileParser(string directory, string filename) {
    ...
    FILE *fp = fopen((directory + filename).c_str(),
    ...
    if (fp == NULL) {
        ...
        fread(str, sizeof(char), numChars,
              fp);  // read the whole file into memory
...
string PdFileParser::nextMessage() {
    if (!isDone) {
        message = line;
        while (!nextLine().empty() && !(line.compare(0, 2, "#X") == 0 ||
                                        line.compare(0, 2, "#N") == 0 ||
...
string PdFileParser::nextLine() {
    ...
    if (newPos == string::npos) { // TODO(mhroth): works?
        ...
        int commaIndex = 0;
        if ((commaIndex = line.find_last_of(",")) != std::string::npos &&
            commaIndex > 0 && line[commaIndex - 1] != '\\') {
...
PdGraph *PdFileParser::execute(PdMessage *initMsg, PdGraph *graph,
                               PdContext *context, bool isSubPatch) {
    ...
    while (!(message = nextMessage()).empty()) {
        ...
        if (!strcmp(hashType, "#N")) {
            ...
            if (!strcmp(objectType, "canvas")) {
                ...
                // subpatch, not an abstraction NOTE(mhroth): pixel location is
                // not recorded
                PdGraph *newGraph = NULL;
                if (graph == NULL) { // if no parent graph exists
                    initMessage->initWithTimestampAndNumElements(
                        0.0, 0); // make a dummy initMessage
                    newGraph =
                        new PdGraph(initMessage, NULL, context,
                                    context->getNextGraphId(), "zg_root");
                    if (!rootPath.empty()) {
                        // inform the root graph of where it is in the file
                        // system, if this information exists. This will allow
                        // abstractions to be correctly loaded.
                    ...
                    if (isSubPatch) {
                        ...
                        newGraph =
                            new PdGraph(graph->getArguments(), graph, context,
                                        graph->getGraphId(), canvasName);
                    } else {
                        // a graph made as an abstraction
                        newGraph = new PdGraph(initMsg, graph, context,
                                               context->getNextGraphId(),
                                               (rootPath + fileName).c_str());
                        isSubPatch = true;
                    }
                    graph->addObject(0, 0,
                                     newGraph); // add the new graph to the
                                                // current one as an object
                }
            ...
            if (!strcmp(objectType, "obj")) {
                ...
                PdMessage::resolveString(
                    objectLabel, graph->getArguments(), 0, resBufferLabel,
                    OBJECT_LABEL_RESOLUTION_BUFFER_LENGTH); // object labels are
                                                            // always strings
                                                            // even if they are
                                                            // numbers, e.g. "1"
                ...
                initMessage->initWithSARb(
                    ...
                    graph->getArguments(), resBuffer, RESOLUTION_BUFFER_LENGTH);

                // create the object
                MessageObject *messageObject =
                    context->newObject(resBufferLabel, initMessage, graph);
                if (messageObject ==
                    NULL) { // object could not be created based on any known
                            // object factory functions
                    if (context->getAbstractionDataBase()->existsAbstraction(
                            objectLabel)) {
                        PdFileParser *parser = new PdFileParser(
                            context->getAbstractionDataBase()->getAbstraction(
                                objectLabel));
                        messageObject =
                            parser->execute(initMessage, graph, context, false);
                        delete parser;
                    } else {
                        string filename = string(objectLabel) + ".pd";
                        string directory =
                            graph->findFilePath(filename.c_str());
                        if (directory.empty()) {
                            // if the system cannot find the file itself, make a
                            // final effort to find the file via the user
                            // supplied callback
                            if (context->callbackFunction != NULL) {
                                char *dir = (char *)context->callbackFunction(
                                    ZG_CANNOT_FIND_OBJECT,
                                    context->callbackUserData, objectLabel);
                                if (dir != NULL) {
                                    // TODO(mhroth): create new object based on
                                    // returned path
                                    free(dir); // free the returned objectpath
                                } else {
                                    context->printErr(
                                        "Unknown object or abstraction '%s'.",
                                        objectLabel);
                                }
                            }
                        }
                        PdFileParser *parser =
                            new PdFileParser(directory, filename);
                        messageObject =
                            parser->execute(initMessage, graph, context, false);
                        delete parser;
                    }
                } else {
                    // add the object to the local graph and make any necessary
                    // registrations
                ...
                if (initMessage->isSymbol(0, "-path")) {
                    ...
                    context->printErr("declare \"%s\" flag is not supported.",
                ...
                context->printStd(
                    "PdFileParser: Replacer array with table, name: '%s'",
                    initMessage->getSymbol(0));
            } else if (!strcmp(objectType, "coords")) {
                continue;
            } else {
                context->printErr("Unrecognised #X object type: \"%s\"",
                                  message.c_str());
            }
            ...
            context->printErr("Unrecognised hash type: \"%s\"",
                              message.c_str());
        }
    }
