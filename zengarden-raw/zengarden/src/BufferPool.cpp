...
BufferPool::~BufferPool() {
    ...
    for (list<std::pair<float *, unsigned int>>::iterator it = reserved.begin();
         it != reserved.end(); ++it) {
...
float *BufferPool::getBuffer(unsigned int numDependencies) {
    ...
    reserved.push_back(
        std::pair<float *, unsigned int>(buffer, numDependencies));
    //  printf("%i/%i buffer used.\n", getNumReservedBuffers(),
    //  getNumTotalBuffers());
    return buffer;
}

void BufferPool::releaseBuffer(float *buffer) {
    // an object may try to release the zero buffer. This should not be
    // possible.
    ...
    // if the buffer is not in the reserved pool, nothing changes. Untracked
    // buffers are left alone.
}

void BufferPool::reserveBuffer(float *buffer, unsigned int reserveCount) {
    if (buffer == zeroBuffer)
        return; // no need to reserve the zero buffer
    ...
    printf("Attempt to reserve unreserved buffer %p +%i.\n  "
           "This may be ok if the buffer is global such as an adc~ input "
           "buffer.\n",
           buffer, reserveCount);
}
...
void BufferPool::resizeBuffers(unsigned int newBufferSize) {
  for (list<std::pair<float *, unsigned int> >::iterator it = reserved.begin();
it != reserved.end(); ++it) { float *buffer = (*it).first; float *newBuffer =
(float *) realloc(buffer, newBufferSize * sizeof(float)); if (newBuffer !=
buffer) { // buffer should start at same address so that it is still 16-byte
aligned free(buffer); free(newBuffer);
