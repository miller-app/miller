...
DspDac::DspDac(PdGraph *graph)
    : DspObject(0, graph->getNumOutputChannels(), 0, 0, graph) {
    // cache pointers to global output buffers in dspBufferAtOutlet
    int numInlets = graph->getNumOutputChannels();
    if (numInlets > 2)
        dspBufferAtOutlet[2] = (float *)calloc(numInlets - 2, sizeof(float *));
...
DspDac::~DspDac() { free(dspBufferAtOutlet[2]); }
...
void DspDac::processSignal(DspObject *dspObject, int fromIndex, int toIndex) {
    ...
    default: {
        ...
        for (int i = 2; i < numDspInlets; i++) {
          ...
          ArrayArithmetic::add(globalOutputBuffer, localDspBufferAtInlet[i],
        globalOutputBuffer, 0, blockSizeInt);
        }
        */
        // allow fallthrough
    }
