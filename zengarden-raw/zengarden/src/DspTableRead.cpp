...
DspTableRead::DspTableRead(PdMessage *initMessage, PdGraph *graph)
...
DspTableRead::~DspTableRead() { free(name); }

void DspTableRead::setTable(MessageTable *aTable) { table = aTable; }
...
void DspTableRead::processDspWithIndex(int fromIndex, int toIndex) {
    if (table != NULL) { // ensure that there is a table to read from!
        ...
        // add the offset
        vDSP_vsadd(dspBufferAtInlet[0] + fromIndex, 1, &offset, outBuff, 1,
                   duration);

        // clip to the bounds of the table
        // NOTE(mhroth): is this necessary? Or does vDSP_vindex clip
        // automatically? What is the clipping behaviour of vDSP_vindex?
        float min = 0;
