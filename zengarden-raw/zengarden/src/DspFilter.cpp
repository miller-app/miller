...
DspFilter::DspFilter(int numMessageInlets, PdGraph *graph)
    : DspObject(numMessageInlets, 1, 0, 1, graph) {
    x1 = x2 = y1 = y2 = 0.0f;
...
void DspFilter::processFilter(DspObject *dspObject, int fromIndex,
                              int toIndex) {
    ...
    for (int i = 2; i < _toIndex; ++i) {
        bufferOut[i] = d->b[0] * bufferIn[i] + d->b[1] * bufferIn[i - 1] +
                       d->b[2] * bufferIn[i - 2] - d->b[3] * bufferOut[i - 1] -
                       d->b[4] * bufferOut[i - 2];
    }
#endif

    memcpy(d->dspBufferAtOutlet[0] + fromIndex, bufferOut + 2,
           n * sizeof(float));
