...
ZGObject *zg_graph_add_new_object(PdGraph *graph, const char *objectString,
    ...
    initMessage->initWithSARb(32, initString, graph->getArguments(),
                              resolutionBuffer, 256);
    MessageObject *messageObject =
        graph->getContext()->newObject(objectLabel, initMessage, graph);
...
ZGConnectionType zg_object_get_connection_type(ZGObject *object,
    ...
    default:
    case MESSAGE:
        return ZG_CONNECTION_MESSAGE;
    case DSP:
        return ZG_CONNECTION_DSP;
    }
}
...
ZGConnectionPair *zg_object_get_connections_at_inlet(ZGObject *object,
                                                     ...
                                                     unsigned int *n) {
    list<ObjectLetPair> connections =
        object->getIncomingConnections(inletIndex);
    *n = connections.size();
    int i = 0;
    ObjectLetPair *conns =
        (ObjectLetPair *)malloc(connections.size() * sizeof(ObjectLetPair));
    for (list<ObjectLetPair>::iterator it = connections.begin();
         it != connections.end(); it++, i++) {
        conns[i] = *it;
    }
...
ZGConnectionPair *zg_object_get_connections_at_outlet(ZGObject *object,
                                                      ...
                                                      unsigned int *n) {
    list<ObjectLetPair> connections =
        object->getOutgoingConnections(outletIndex);
    *n = connections.size();
    int i = 0;
    ObjectLetPair *conns =
        (ObjectLetPair *)malloc(connections.size() * sizeof(ObjectLetPair));
...
void zg_object_send_message(MessageObject *object, unsigned int inletIndex,
                            ZGMessage *message) {
...
ZGContext *zg_context_new(int numInputChannels, int numOutputChannels,
                          ...
                          void *(*callbackFunction)(ZGCallbackFunction, void *,
                                                    void *),
                          void *userData) {
    return new PdContext(numInputChannels, numOutputChannels, blockSize,
                         sampleRate, callbackFunction, userData);
}

void zg_context_delete(ZGContext *context) { delete context; }

ZGGraph *zg_context_new_empty_graph(PdContext *context) {
    PdMessage *initMessage = PD_MESSAGE_ON_STACK(
    ...
    // with a unique id
    PdGraph *graph = new PdGraph(initMessage, NULL, context,
                                 context->getNextGraphId(), "zg_free");
    return graph;
}

ZGGraph *zg_context_new_graph_from_file(PdContext *context,
                                        const char *directory,
                                        const char *filename) {
    PdFileParser *parser =
        new PdFileParser(string(directory), string(filename));
    PdGraph *graph = parser->execute(context);
    graph->addDeclarePath(directory); // ensure that the root director is added
                                      // to the declared path set
    delete parser;
    return graph;
}

ZGGraph *zg_context_new_graph_from_string(PdContext *context,
                                          const char *netlist) {
...
void zg_context_process(PdContext *context, float *inputBuffers,
                        float *outputBuffers) {
...
void zg_context_process_s(ZGContext *context, short *inputBuffers,
...
#if __APPLE__
    // convert short to float, and uninterleave the samples into the float
    // buffer allow fallthrough in all cases
    switch (numInputChannels) {
    default: { // input channels > 2
        for (int i = 2; i < numInputChannels; ++i) {
            vDSP_vflt16(inputBuffers + i, numInputChannels,
                        finputBuffers + i * blockSize, 1, blockSize);
        } // allow fallthrough
    }
    case 2:
        vDSP_vflt16(inputBuffers + 1, numInputChannels,
                    finputBuffers + blockSize, 1, blockSize);
    case 1:
        vDSP_vflt16(inputBuffers, numInputChannels, finputBuffers, 1,
                    blockSize);
    case 0:
        break;
    }
    ...
    float min = -1.0f;
    float max = 1.0f;
    vDSP_vclip(foutputBuffers, 1, &min, &max, foutputBuffers, 1,
               outputBufferLength);
    ...
    default: { // output channels > 2
        for (int i = 2; i < numOutputChannels; ++i) {
            vDSP_vfix16(foutputBuffers + i * blockSize, numOutputChannels,
                        outputBuffers + i, 1, blockSize);
        } // allow fallthrough
    }
    case 2:
        vDSP_vfix16(foutputBuffers + blockSize, 1, outputBuffers + 1,
                    numOutputChannels, blockSize);
    case 1:
        vDSP_vfix16(foutputBuffers, 1, outputBuffers, numOutputChannels,
                    blockSize);
    case 0:
        break;
    }
#else
    // uninterleave and short->float the samples in inputBuffers to
    // finputBuffers
    switch (numInputChannels) {
    default: {
    ...
    default: {
        for (int k = 2; k < numOutputChannels; k++) {
            for (int i = k, j = k * blockSize; i < outputBufferLength;
    ...
    case 2: {
        for (int i = 1, j = blockSize; i < outputBufferLength;
             i += numOutputChannels, j++) {
            outputBuffers[i] = (short)(foutputBuffers[j] * 32767.0f);
        } // allow fallthrough
    }
    case 1: {
        for (int i = 0, j = 0; i < outputBufferLength;
             i += numOutputChannels, j++) {
            outputBuffers[i] = (short)(foutputBuffers[j] * 32767.0f);
        } // allow fallthrough
    }
...
void zg_context_register_receiver(ZGContext *context,
...
void zg_context_send_messageV(PdContext *context, const char *receiverName,
...
void zg_context_send_message_at_blockindex(PdContext *context,
                                           const char *receiverName,
                                           double blockIndex,
    ...
    if (blockIndex >= 0.0 &&
        blockIndex <= (double)(context->getBlockSize() - 1)) {
        timestamp += blockIndex / context->getSampleRate();
    }
    context->scheduleExternalMessageV(receiverName, timestamp, messageFormat,
                                      ap);
    va_end(ap);
}

void zg_context_send_midinote(PdContext *context, int channel, int noteNumber,
    ...
    zg_context_send_message_at_blockindex(context, receiverName, blockIndex,
                                          "fff", (float)noteNumber,
                                          (float)velocity, (float)channel);

    // all message are also sent to the omni listener
    zg_context_send_message_at_blockindex(context, "zg_notein_omni", blockIndex,
                                          "fff", (float)noteNumber,
                                          (float)velocity, (float)channel);
}
...
unsigned int zg_graph_get_dollar_zero(ZGGraph *graph) {
    return (graph != NULL) ? (unsigned int)graph->getArguments()->getFloat(0)
                           : 0;
}
...
ZGObject **zg_graph_get_objects(ZGGraph *graph, unsigned int *n) {
    ...
    *n = (unsigned int)nodeList.size();
    ZGObject **nodeArray =
        (ZGObject **)malloc(nodeList.size() * sizeof(ZGObject *));
...
void zg_table_set_buffer(MessageObject *table, float *buffer, unsigned int n) {
    if (table != NULL && table->getObjectType() == MESSAGE_TABLE) {
        ...
        float *tableBuffer = messageTable->resizeBuffer(
            n); // resize the buffer to the new size (if necessary)
        memcpy(
            tableBuffer, buffer,
            n * sizeof(float)); // copy the contents of the buffer to the table
...
ZGMessage *zg_message_new_from_string(double timetamp, const char *initString) {
    ...
    // make a local copy of the initString so that strtok in initWithString
    // won't break it
    char str[strlen(initString) + 1];
...
void zg_message_set_symbol(PdMessage *message, unsigned int index,
                           const char *s) {
...
ZGMessageElementType zg_message_get_element_type(PdMessage *message,
                                                 unsigned int index) {
    switch (message->getType(index)) {
    case FLOAT:
        return ZG_MESSAGE_ELEMENT_FLOAT;
    case SYMBOL:
        return ZG_MESSAGE_ELEMENT_SYMBOL;
    default:
...
char *zg_message_to_string(ZGMessage *message) { return message->toString(); }
