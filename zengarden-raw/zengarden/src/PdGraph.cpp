...
// a PdGraph begins with zero inlets and zero outlets. These will be added as
// inlet/~ and outlet/~ objects are added to the graph
PdGraph::PdGraph(PdMessage *initMessage, PdGraph *parentGraph,
                 PdContext *context, int graphId, const char *graphName)
    : DspObject(0, 0, 0, 0,
                (parentGraph == NULL) ? context->getBlockSize()
                                      : parentGraph->getBlockSize(),
                parentGraph) {
    ...
    // all graphs start out unattached to any context, though they exist in a
    // context
    ...
    memcpy(graphArguments->getElement(1), initMessage->getElement(0),
           numInitElements * sizeof(MessageAtom));
...
PdGraph::~PdGraph() {
    ...
    for (list<DspObject *>::iterator it = dspNodeList.begin();
         it != dspNodeList.end(); ++it) {
        DspObject *dspObject = *it;

        if (dspObject->getGraph() != this)
            break;
        if (!strcmp(dspObject->toString().c_str(),
                    DspImplicitAdd::getObjectLabel())) {
            delete dspObject;
        }
    }

    // delete all constituent nodes
    for (list<MessageObject *>::iterator it = nodeList.begin();
         it != nodeList.end(); ++it) {
        delete *it;
    }
}
...
void PdGraph::addObject(float canvasX, float canvasY,
                        MessageObject *messageObject) {
    lockContextIfAttached();

    nodeList.push_back(
        messageObject); // all nodes are added to the node list regardless
    ...
    while (it != end) {
        ...
        if (*it == object) {
            ...
            for (int i = 0; i < object->getNumInlets(); i++) {
                list<ObjectLetPair> incomingConnections =
                    object->getIncomingConnections(i);
            ...
            for (int i = 0; i < object->getNumOutlets(); i++) {
                list<ObjectLetPair> outgoingConnections =
                    object->getOutgoingConnections(i);
            ...
            // remove the object from the dspNodeList if the object processes
            // audio
            if (object->doesProcessAudio()) {
                dspNodeList.remove((DspObject *)object);
            }

            // remove the object from any special lists if it is in any of them
            // (e.g., receive, throw~, etc.)
...
void PdGraph::addLetObjectToLetList(MessageObject *inletObject,
                                    float newPosition,
                                    vector<MessageObject *> *letList) {
...
void PdGraph::registerObject(MessageObject *messageObject) {
    ...
    case MESSAGE_NOTEIN: {
        context->registerRemoteMessageReceiver(
            reinterpret_cast<RemoteMessageReceiver *>(messageObject));
        break;
    }
    case MESSAGE_TABLE: {
        // tables must be registered globally as a table, but can also receive
        // remote messages
        context->registerRemoteMessageReceiver(
            reinterpret_cast<RemoteMessageReceiver *>(messageObject));
    ...
    case MESSAGE_TABLE_READ: {
        context->registerTableReceiver(
            reinterpret_cast<MessageTableRead *>(messageObject));
        break;
    }
    case MESSAGE_TABLE_WRITE: {
        context->registerTableReceiver(
            reinterpret_cast<MessageTableWrite *>(messageObject));
        break;
    }
    case DSP_CATCH: {
    ...
    case DSP_TABLE_WRITE: {
        context->registerTableReceiver(
            reinterpret_cast<DspTableWrite *>(messageObject));
        break;
    }
    case DSP_THROW: {
...
void PdGraph::unregisterObject(MessageObject *messageObject) {
    ...
    case MESSAGE_RECEIVE:
    case MESSAGE_NOTEIN: {
        context->unregisterRemoteMessageReceiver(
            (RemoteMessageReceiver *)messageObject);
        break;
    }
    case MESSAGE_TABLE_READ: {
...
char *PdGraph::resolveFullPath(const char *filename) {
    if (DeclareList::isFullPath(filename)) {
        return StaticUtils::fileExists(filename)
                   ? StaticUtils::copyString(filename)
                   : NULL;
    } else {
        string directory = findFilePath(filename);
        return (!directory.empty())
                   ? StaticUtils::concatStrings(directory.c_str(), filename)
                   : NULL;
    }
}

string PdGraph::findFilePath(const char *filename) {
    for (list<string>::iterator it = declareList->getIterator();
         it != declareList->getEnd(); ++it) {
        string directory = *it;
...
PdMessage *PdGraph::scheduleMessage(MessageObject *messageObject,
                                    int outletIndex, PdMessage *message) {
    return context->scheduleMessage(messageObject, outletIndex, message);
}

void PdGraph::cancelMessage(MessageObject *messageObject, int outletIndex,
                            PdMessage *message) {
...
void PdGraph::processGraph(DspObject *dspObject, int fromIndex, int toIndex) {
    ...
    if (d->switched) {
        // when inlets are processed, they will resolve their buffers and
        ...
        for (list<DspObject *>::iterator it = d->dspNodeList.begin();
             it != d->dspNodeList.end(); ++it) {
            DspObject *dspObject = *it;
...
void PdGraph::addConnection(MessageObject *fromObject, int outletIndex,
                            MessageObject *toObject, int inletIndex) {
    // check to make sure that this connection can even work. Otherwise don't
    // bother.
    if (outletIndex >= fromObject->getNumOutlets() ||
        inletIndex >= toObject->getNumInlets()) {
        printErr("mismatched connnection. Attempt to make a connection from "
                 ...
                 fromObject->getNumOutlets(), toObject->toString().c_str(),
                 toObject, inletIndex, toObject->getNumInlets());
        return;
    }

    lockContextIfAttached();
    toObject->addConnectionFromObjectToInlet(fromObject, outletIndex,
                                             inletIndex);
    fromObject->addConnectionToObjectFromOutlet(toObject, inletIndex,
                                                outletIndex);

    // NOTE(mhroth): very heavy handed approach. Always recompute the process
    // order when adding connections. In theory this function should check to
    // see if a reordering is even necessary and then only make the appropriate
    // changes. Usually a complete reevaluation shouldn't be necessary, and
    // otherwise the use of a linked list to store the node list should make the
    // reordering fast.
...
void PdGraph::addConnection(int fromObjectIndex, int outletIndex,
                            int toObjectIndex, int inletIndex) {
    list<MessageObject *>::iterator fromIt = nodeList.begin();
    for (int i = 0; i < fromObjectIndex; i++)
        fromIt++;
    list<MessageObject *>::iterator toIt = nodeList.begin();
    for (int i = 0; i < toObjectIndex; i++)
        toIt++;
...
/*
 * removeConnection does not force a reordering of the dspNodeList, as lost
 * connections do not create any new constraints on the dsp object ordering that
 * weren't there already. addConnection does force a reevaluation, as the new
 * connection may force a new constrained how how objects are ordered.
 */
void PdGraph::removeConnection(MessageObject *fromObject, int outletIndex,
                               MessageObject *toObject, int inletIndex) {
    lockContextIfAttached();
    toObject->removeConnectionFromObjectToInlet(fromObject, outletIndex,
                                                inletIndex);
    fromObject->removeConnectionToObjectFromOutlet(toObject, inletIndex,
                                                   outletIndex);
    unlockContextIfAttached();
}
...
void PdGraph::addConnectionFromObjectToInlet(MessageObject *messageObject,
    ...
    case MESSAGE: {
        ...
        if (inletObject->getObjectType() == MESSAGE_INLET) {
            MessageInlet *messageInlet =
                reinterpret_cast<MessageInlet *>(inletObject);
            messageInlet->addConnectionFromObjectToInlet(messageObject,
                                                         outletIndex, 0);
        } else {
            printErr("Connection [%s]:%i is of type DSP and cannot be "
                     "connected to inlet.",
    ...
    case DSP: {
        ...
        if (inletObject->getObjectType() == DSP_INLET) {
            ...
            dspInlet->addConnectionFromObjectToInlet(messageObject, outletIndex,
                                                     0);
        } else {
            printErr("Connection [%s]:%i is of type MESSAGE and cannot be "
                     "connected to inlet~.",
                     messageObject->getObjectLabel(), outletIndex);
        }
        break;
...
void PdGraph::addConnectionToObjectFromOutlet(MessageObject *messageObject,
                                              int inletIndex, int outletIndex) {
    switch (getConnectionType(outletIndex)) {
    case MESSAGE: {
        MessageOutlet *messageOutlet =
            reinterpret_cast<MessageOutlet *>(outletList.at(outletIndex));
        messageOutlet->addConnectionToObjectFromOutlet(messageObject,
                                                       inletIndex, 0);
        break;
    }
    case DSP: {
        DspOutlet *dspOutlet =
            reinterpret_cast<DspOutlet *>(outletList.at(outletIndex));
        dspOutlet->addConnectionToObjectFromOutlet(messageObject, inletIndex,
                                                   0);
        break;
    }
    default:
...
void PdGraph::removeConnectionFromObjectToInlet(MessageObject *messageObject,
                                                int outletIndex,
                                                int inletIndex) {
    // NOTE(mhroth): double check! this should work even if inletObject is a
    // DspInlet
    MessageObject *inletObject = inletList.at(inletIndex);
    inletObject->removeConnectionFromObjectToInlet(messageObject, outletIndex,
                                                   0);
}

void PdGraph::removeConnectionToObjectFromOutlet(MessageObject *messageObject,
                                                 int inletIndex,
                                                 int outletIndex) {
    MessageObject *outletObject = outletList.at(outletIndex);
    outletObject->removeConnectionToObjectFromOutlet(messageObject, inletIndex,
                                                     0);
}
...
void PdGraph::setDspBufferAtOutlet(float *buffer, unsigned int outletIndex) {
    // nothing to do because DspOutlet objects do not allow setting outlet
    // buffers
}
...
list<DspObject *> PdGraph::getProcessOrder() {
    if (isOrdered) {
        ...
        for (vector<MessageObject *>::iterator it = inletList.begin();
             it != inletList.end(); ++it) {
            MessageObject *messageObject = *it;
            // NOTE(mhroth): try to use some "GraphInlet" interface here
            switch (messageObject->getObjectType()) {
            case MESSAGE_INLET: {
                MessageInlet *messgeInlet =
                    reinterpret_cast<MessageInlet *>(messageObject);
                list<DspObject *> inletProcessOrder =
                    messgeInlet->getProcessOrderFromInlet();
                processOrder.splice(processOrder.end(), inletProcessOrder);
                break;
            }
            case DSP_INLET: {
                DspInlet *dspInlet =
                    reinterpret_cast<DspInlet *>(messageObject);
                list<DspObject *> inletProcessOrder =
                    dspInlet->getProcessOrderFromInlet();
                processOrder.splice(processOrder.end(), inletProcessOrder);
                break;
            }
            default:
...
void PdGraph::computeDeepLocalDspProcessOrder() {
     ...
     * Find all leaf nodes in nodeList. this includes PdGraphs as they are
     * objects as well. For each leaf node, generate an ordering for all of the
     * nodes in the current graph. the basic idea is to compute the full process
     * order in each subgraph. Finally, all non-dsp nodes must be removed from
     * this list in order to derive the dsp process order.
     */
    ...
    for (list<MessageObject *>::iterator it = nodeList.begin();
        ...
        object
            ->resetOrderedFlag(); // reset the ordered flag on all local objects
        if (object->isLeafNode()) { // isLeafNode() takes into account send/~
                                    // and throw~ objects
    ...
    for (list<MessageObject *>::iterator it = leafNodeList.begin();
         it != leafNodeList.end(); ++it) {
        MessageObject *object = *it;
    ...
    if (!dspNodeList.empty()) {
      // print dsp evaluation order for debugging, but only if there are any
    nodes to list printStd("  - ordered evaluation list ---"); list<DspObject
    *>::iterator it = dspNodeList.begin(); list<DspObject *>::iterator end =
    dspNodeList.end(); while (it != end) {
...
double PdGraph::getBlockIndex(PdMessage *message) {
    ...
    return (message->getTimestamp() - context->getBlockStartTimestamp()) *
           0.001 * context->getSampleRate();
}

float PdGraph::getSampleRate() {
    // there is no such thing as a local sample rate. Return the sample rate of
    // the context.
    return context->getSampleRate();
}

int PdGraph::getGraphId() { return graphId; }
...
PdMessage *PdGraph::getArguments() { return graphArguments; }

int PdGraph::getNumInputChannels() { return context->getNumInputChannels(); }

int PdGraph::getNumOutputChannels() { return context->getNumOutputChannels(); }

int PdGraph::getBlockSize() { return blockSizeInt; }

bool PdGraph::isRootGraph() { return (parentGraph == NULL); }

MessageTable *PdGraph::getTable(char *name) { return context->getTable(name); }
...
bool PdGraph::doesProcessAudio() {
    ...
    // This works because graph objects are only created after they have been
    // filled with objects.
...
void PdGraph::setBlockSize(int blockSize) {
    ...
    if (blockSize <= parentGraph->getBlockSize()) {
        ...
        // notify all dsp objects and get them to resize their buffers
        // accordingly
    }
}

PdGraph *PdGraph::getParentGraph() { return parentGraph; }

void PdGraph::setSwitch(bool switched) { this->switched = switched; }

bool PdGraph::isSwitchedOn() { return switched; }
...
unsigned int PdGraph::getNumInlets() { return inletList.size(); }

unsigned int PdGraph::getNumOutlets() { return outletList.size(); }

PdContext *PdGraph::getContext() { return context; }

list<MessageObject *> PdGraph::getNodeList() { return nodeList; }

BufferPool *PdGraph::getBufferPool() { return context->getBufferPool(); }
