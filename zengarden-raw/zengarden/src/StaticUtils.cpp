...
char *StaticUtils::concatStrings(const char *path0, const char *path1) {
    if (path0 == NULL || path1 == NULL) {
        ...
        char *joinedPaths =
            (char *)malloc((strlen(path0) + strlen(path1) + 1) * sizeof(char));
...
float StaticUtils::sineApprox(float x) {
    ...
    // NOTE(mhroth): use for former method instead of the latter for now (unless
    // performance issues arise) in order to avoid type-punning pointer
    // warnings.
    // int y = *(int *)&x | 0x80000000;
    // return (1.273239544735163f * x) - (0.405284734569351f * x) * (*(float
    // *)&y);
}
