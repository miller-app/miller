...
DspSqrt::DspSqrt(PdMessage *initMessage, PdGraph *graph)
...
void DspSqrt::processSignal(DspObject *dspObject, int fromIndex, int toIndex) {
    ...
    switch (toIndex & 0x3) {
    case 3:
        *outBuff++ = (*inBuff > 0.0f) ? sqrtf(*inBuff) : 0.0f;
        ++inBuff;
    case 2:
        *outBuff++ = (*inBuff > 0.0f) ? sqrtf(*inBuff) : 0.0f;
        ++inBuff;
    ...
    while (n4) {
        ...
        // NOTE(mhroth): is this really necessary? What does _mm_sqrt_ps return
        // otherwise?
