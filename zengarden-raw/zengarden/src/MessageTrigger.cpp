...
MessageObject *MessageTrigger::newObject(PdMessage *initMessage,
                                         PdGraph *graph) {
...
MessageTrigger::MessageTrigger(PdMessage *initMessage, PdGraph *graph)
    ...
    // resolve the symbols to type in a copy of the original message on the
    // stack. That way the symbol pointers don't get lost when replace with new
    // MessageAtom types.
    ...
    memcpy(message->getElement(0), initMessage->getElement(0),
           numElements * sizeof(MessageAtom));
    message->resolveSymbolsToType();
    castMessage = message->copyToHeap();
}

MessageTrigger::~MessageTrigger() { castMessage->freeMessage(); }
...
void MessageTrigger::processMessage(int inletIndex, PdMessage *message) {
    ...
    for (int i = numMessageOutlets - 1; i >= 0;
         i--) { // send messages from outlets right-to-left
        // TODO(mhroth): There is currently no support for converting to a LIST
        // type
        ...
        case FLOAT: {
            ...
            case FLOAT: {
                outgoingMessage->initWithTimestampAndFloat(
                    message->getTimestamp(), message->getFloat(0));
                sendMessage(i, outgoingMessage);
                break;
            }
            case SYMBOL: {
                outgoingMessage->initWithTimestampAndSymbol(
                    message->getTimestamp(), (char *)"float");
                sendMessage(i, outgoingMessage);
                break;
            }
            case BANG: {
        ...
        case SYMBOL: {
            ...
            case ANYTHING: {
                outgoingMessage->initWithTimestampAndSymbol(
                    message->getTimestamp(), message->getSymbol(0));
                sendMessage(i, outgoingMessage);
                break;
            }
            case SYMBOL: {
        ...
        case BANG: {
            ...
            case FLOAT: {
                outgoingMessage->initWithTimestampAndFloat(
                    message->getTimestamp(), 0.0f);
                sendMessage(i, outgoingMessage);
                break;
            }
            case SYMBOL: {
                outgoingMessage->initWithTimestampAndSymbol(
                    message->getTimestamp(), (char *)"symbol");
                sendMessage(i, outgoingMessage);
                break;
            }
            case ANYTHING:
