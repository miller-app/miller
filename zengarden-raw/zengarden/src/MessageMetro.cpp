...
MessageMetro::MessageMetro(PdMessage *initMessage, PdGraph *graph)
    : MessageObject(2, 1, graph) {
    // default to interval of one second
    intervalInMs =
        initMessage->isFloat(0) ? (double)initMessage->getFloat(0) : 1000.0;
    pendingMessage = NULL;
}
...
void MessageMetro::sendMessage(int outletIndex, PdMessage *message) {
    // schedule the pending message before the current one is sent so that if a
    // stop message arrives at this object while in this function, then the next
    // message can be cancelled
    pendingMessage = PD_MESSAGE_ON_STACK(1);
    pendingMessage->initWithTimestampAndBang(message->getTimestamp() +
                                             intervalInMs);
...
void MessageMetro::startMetro(double timestamp) {
    // Ensure that there is no pending message for this metro. If there is, then
    // cancel it. This allows a metro to be banged multiple times and always
    // restart the timing from the most recently received bang.
    stopMetro();
