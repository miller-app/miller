...
MessageObject *DspVariableLine::newObject(PdMessage *initMessage,
                                          PdGraph *graph) {
    return new DspVariableLine(initMessage, graph);
}

DspVariableLine::DspVariableLine(PdMessage *initMessage, PdGraph *graph)
    : DspObject(3, 0, 0, 1, graph) {
...
DspVariableLine::~DspVariableLine() {
    // because all messages are stored in (and thus owned by) the context, it is
    // not necessary to delete pending messages here
}
...
void DspVariableLine::processMessage(int inletIndex, PdMessage *message) {
    ...
    case 0: {
        if (message->isFloat(0)) {
            ...
            // clear all messages after the given start time, insert the new
            // message into the list
            PdMessage *controlMessage = PD_MESSAGE_ON_STACK(2);
            controlMessage->initWithTimestampAndNumElements(
                message->getTimestamp() + delay, 2);
            ...
            if (delay == 0.0f) {
                ...
                PdMessage *heapMessage =
                    graph->scheduleMessage(this, 0, controlMessage);
    ...
    default: {
        graph->printErr(
            "vline~ does not respond to messages on 2nd and 3rd inlets. "
...
void DspVariableLine::clearAllMessagesAtOrAfter(double timestamp) {
    for (list<PdMessage *>::iterator it = messageList.begin();
...
void DspVariableLine::updatePathWithMessage(PdMessage *message) {
    if (message == NULL) {
        ...
        numSamplesToTarget = StaticUtils::millisecondsToSamples(
            message->getFloat(1), graph->getSampleRate());
...
void DspVariableLine::sendMessage(int outletIndex, PdMessage *message) {
    // this message should be at the front of the messageList, but we use the
    // more general remove function just in case
    messageList.remove(message);

    updatePathWithMessage(message);
}

// NOTE(mhroth): this code could be improved to be sub-sample accurate with
// regards to calculating last sample output
void DspVariableLine::processSignal(DspObject *dspObject, int fromIndex,
                                    int toIndex) {
    DspVariableLine *d = reinterpret_cast<DspVariableLine *>(dspObject);

    if (d->numSamplesToTarget <= 0.0f) {
        ArrayArithmetic::fill(d->dspBufferAtOutlet[0], d->lastOutputSample,
                              fromIndex, toIndex);
    } else {
...
#if __APPLE__
            vDSP_vramp(&(d->lastOutputSample), &(d->slope),
                       d->dspBufferAtOutlet[0] + fromIndex, 1, n);
#else

#endif

            d->lastOutputSample =
                d->dspBufferAtOutlet[0][toIndex - 1] + d->slope;
            d->numSamplesToTarget -= n;
        } else {
// must update slope in this buffer
#if __APPLE__
            vDSP_vramp(&(d->lastOutputSample), &(d->slope),
                       d->dspBufferAtOutlet[0] + fromIndex, 1,
                       (int)d->numSamplesToTarget);
#else
