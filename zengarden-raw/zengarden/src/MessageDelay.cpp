...
MessageDelay::MessageDelay(PdMessage *initMessage, PdGraph *graph)
...
void MessageDelay::processMessage(int inletIndex, PdMessage *message) {
    ...
    case 0: {
        ...
        case BANG: {
            ...
            scheduledMessage->initWithTimestampAndBang(message->getTimestamp() +
                                                       delayMs);
            scheduledMessage =
    ...
    case 1: {
        if (message->isFloat(0)) {
            // if an outstanding message exists when the delay is reset, the
...
void MessageDelay::sendMessage(int outletIndex, PdMessage *message) {
    if (message == scheduledMessage) {
        // now that we know that this message is being sent, we don't have to
