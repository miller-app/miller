...
MessageObject *DspTableRead4::newObject(PdMessage *initMessage,
                                        PdGraph *graph) {
    return new DspTableRead4(initMessage, graph);
}

DspTableRead4::DspTableRead4(PdMessage *initMessage, PdGraph *graph)
    : DspObject(2, 1, 0, 1, graph) {
...
DspTableRead4::~DspTableRead4() { free(name); }

void DspTableRead4::setTable(MessageTable *aTable) { table = aTable; }
...
void DspTableRead4::processDspWithIndex(int fromIndex, int toIndex) {
    if (table != NULL) { // ensure that there is a table to read from!
        ...
        // vDSP_vclip(inputBuffer+startSampleIndex, 1, &zero,
        // &bufferLengthFloat,
        //    inputBuffer+startSampleIndex, 1, endSampleIndex-startSampleIndex);
        // NOTE(mhroth): is isn't clear what the clipping behaviour of
        // vDSP_vlint is, but I *think* that it is doing the right thing (i.e.,
        // clipping OOB indicies)
        int duration = toIndex - fromIndex;
        vDSP_vsadd(dspBufferAtInlet[0] + fromIndex, 1, &offset,
                   dspBufferAtOutlet[0] + fromIndex, 1, duration);
        vDSP_vlint(buffer, dspBufferAtOutlet[0] + fromIndex, 1,
                   dspBufferAtOutlet[0] + fromIndex, 1, duration, bufferLength);
#else
