...
DspDelayRead::DspDelayRead(PdMessage *initMessage, PdGraph *graph)
    : DelayReceiver(1, 0, 0, 1, graph) {
    if (initMessage->isSymbol(0) && initMessage->isFloat(1)) {
        name = StaticUtils::copyString(initMessage->getSymbol(0));
        delayInSamples = StaticUtils::millisecondsToSamples(
            initMessage->getFloat(1), graph->getSampleRate());
    } else {
        graph->printErr("delread~ must be initialised in the format [delread~ "
                        "name delay].");
        delayInSamples = 0.0f;
    }
...
void DspDelayRead::onInletConnectionUpdate(unsigned int inletIndex) {
    //  processFunction = (incomingMessageConnections[0].size() > 0) ?
    //  &processScalar : &processSignal;
}
...
void DspDelayRead::processMessage(int inletIndex, PdMessage *message) {
    if (inletIndex == 0 && message->isFloat(0)) {
        ...
        delayInSamples = StaticUtils::millisecondsToSamples(
            message->getFloat(0), graph->getSampleRate());
    }
}

void DspDelayRead::processSignal(DspObject *dspObject, int fromIndex,
                                 int toIndex) {
    ...
    if (delayIndex > bufferLength - toIndex) {
        int samplesInBuffer =
            bufferLength - delayIndex; // samples remaining in the buffer that
                                       // belong in this block
        memcpy(d->dspBufferAtOutlet[0], buffer + delayIndex,
               samplesInBuffer * sizeof(float));
        memcpy(d->dspBufferAtOutlet[0] + samplesInBuffer, buffer,
               (toIndex - samplesInBuffer) * sizeof(float));
    } else {
        memcpy(d->dspBufferAtOutlet[0], buffer + delayIndex,
