...
// it might nice if this class were implemented using a hashtable with receiver
// name as the key and Lists as the value.
MessageSendController::MessageSendController(PdContext *aContext)
    : MessageObject(0, 0, NULL) {
    context = aContext;
...
int MessageSendController::getNameIndex(const char *receiverName) {
    if (!strcmp("pd", receiverName)) {
        return SYSTEM_NAME_INDEX; // a special case for sending messages to the
                                  // system
    }

    for (int i = 0; i < sendStack.size(); i++) {
        string str = sendStack[i].first;
        if (!str.compare(receiverName))
            return i;
    }
    return -1;
}

void MessageSendController::receiveMessage(const char *name,
                                           PdMessage *message) {
    ...
    // check to see if the receiver name has been registered as an external
    // receiver
    if (externalReceiverSet.find(string(name)) != externalReceiverSet.end()) {
        std::pair<const char *, PdMessage *> pair = make_pair(name, message);
        context->callbackFunction(ZG_RECEIVER_MESSAGE,
                                  context->callbackUserData, &pair);
    }
}
...
void MessageSendController::sendMessage(int outletIndex, PdMessage *message) {
    if (outletIndex == SYSTEM_NAME_INDEX) {
        ...
        set<RemoteMessageReceiver *> receiverSet =
            sendStack[outletIndex].second;
        for (set<RemoteMessageReceiver *>::iterator it = receiverSet.begin();
...
void MessageSendController::removeReceiver(RemoteMessageReceiver *receiver) {
    ...
    if (nameIndex != -1) {
        set<RemoteMessageReceiver *> *receiverSet =
            &(sendStack[nameIndex].second);
        receiverSet->erase(receiver);
        // NOTE(mhroth):
        // once the receiver set has been created, it should not be erased
        // anymore from the sendStack. PdContext depends on the nameIndex to be
        // constant for all receiver names once they are defined, as a message
        // destined for that receiver may already be in the message queue with
        // the given index. If the indicies change, then message will be sent to
        // the wrong receiver set. This is a stupid constraint. Fix it.
    }
}
...
void MessageSendController::unregisterExternalReceiver(
