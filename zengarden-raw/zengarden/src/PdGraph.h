...
class PdGraph : public DspObject {
  ...
  public:
    ...
    PdGraph(PdMessage *initMessage, PdGraph *graph, PdContext *context,
            int graphId, const char *graphName);
    ~PdGraph();

    /**
     * Schedules a <code>PdMessage</code> to be sent by the
     * <code>MessageObject</code> from the <code>outletIndex</code> at the
     * specified <code>time</code>.
     */
    PdMessage *scheduleMessage(MessageObject *messageObject, int outletIndex,
                               PdMessage *message);

    /** Cancel a scheduled <code>PdMessage</code> according to its id. */
    void cancelMessage(MessageObject *messageObject, int outletIndex,
                       PdMessage *message);

    /*
     * Messages arriving at <code>PdGraph</code>s are processed immediately
     * (passed on to inlet objects, unlike with super-<code>DspObject</code>
     * objects.
     */
...
#pragma mark - Add/Remove Connection
    ...
    /** Connect the given <code>MessageObject</code>s from the given outlet to
     * the given inlet. */
    void addConnection(int fromObjectIndex, int outletIndex, int toObjectIndex,
                       int inletIndex);
    void addConnection(MessageObject *fromObject, int outletIndex,
                       MessageObject *toObject, int inletIndex);
    void removeConnection(MessageObject *fromObject, int outletIndex,
                          MessageObject *toObject, int inletIndex);
...
#pragma mark -
    ...
    /** Returns <code>true</code> if the audio processing of this graph is
    ...
    /** Returns <code>true</code> of this graph has no parents,
     * code>false</code> otherwise. */
    bool isRootGraph();

    /** Returns this graph's parent graph. Returns <code>NULL</code> if this
     * graph is a top-level graph. */
    ...
    /** Get the argument list in the form of a <code>PdMessage</code> from the
     * graph. */
    ...
    /** Returns the global dsp buffer at the given inlet. Exclusively used by
     * <code>DspAdc</code>. */
    float *getGlobalDspBufferAtInlet(int inletIndex);

    /** Returns the global dsp buffer at the given outlet. Exclusively used by
     * <code>DspDac</code>. */
    ...
    /** Computes the local tree and node processing ordering for dsp nodes,
     * including subgraphs. */
    void computeDeepLocalDspProcessOrder();

    /**
     * Get the process order as if this object (i.e. graph) were an atomic
     * object. The internal process order is not changed.
     */
    list<DspObject *> getProcessOrder();
    bool isLeafNode();

    /**
     * Sends the given message to all [receive] objects with the given
     * <code>name</code>. This function is used by message boxes to send
     * messages described be the syntax:
     * ;
     * name message;
     */
    ...
    /** Add an object to the graph, taking care of any special object
     * registration. */
    void addObject(float canvasX, float canvasY, MessageObject *node);

    /**
     * Remove the object from the graph, also removing all of the connections to
     * and from ths object. The object is then deleted. The object reference is
     * invalid after calling this function and it is an error to reuse it.
     */
    ...
    /**
     * Searches all declared paths to find a file matching the given name. The
     * given filename should be a relative path, NOT a full path. Returns the
     * directory in which the file was found, or NULL if nothing could be found.
     * The returned string SHOULD NOT be free()ed by the caller. It belongs to
     * the DeclareList.
     */
    string findFilePath(const char *filename);

    /**
     * Resolves the full path of the given file. If the file is already fully
     * specified then a copy of the string is returned. Otherwise all declared
     * paths are searched and the full path is returned. The returned path
     * SHOULD be freed by the caller.
     */
    char *resolveFullPath(const char *filename);

    /**
     * Adds a full or partial path to the declare list. If it is a relative
     * path, then it will be resolved relative to the path of the abstraction.
     * If this graph is a subgraph (not an abstraction), then the path will be
     * handed off to the parent graph to be handled.
     */
  ...
  private:
    ...
    MessageObject *newObject(char *objectType, char *objectLabel,
    ...
    void addLetObjectToLetList(MessageObject *inletObject, float newPosition,
    ...
    /**
     * A boolean indicating if this graph is currently attached to a context. It
     * is automatically updated when this graph or a super graph is added or
    ...
    /** The list of arguments to the graph. Stored as a <code>PdMessage</code>
     * for simplicity. */
    PdMessage *graphArguments;

    /** True if the graph is switch on and should process audio. False
     * otherwise. */
    bool switched;
    ...
    /**
     * A list of all <code>DspObject</code>s in this graph, in the order in
     * which they should be called in the <code>processFunction()</code> loop.
     */
    list<DspObject *> dspNodeList;

    /** A list of all inlet (message or audio) nodes in this subgraph. */
    vector<MessageObject *>
        inletList; // in fact contains only MessageInlet and DspInlet objects

    /** A list of all outlet (message or audio) nodes in this subgraph. */
    vector<MessageObject *>
        outletList; // in fact contains only MessageOutlet and DspOutlet objects
