...
MessageObject *DspVariableDelay::newObject(PdMessage *initMessage,
                                           PdGraph *graph) {
    return new DspVariableDelay(initMessage, graph);
}

DspVariableDelay::DspVariableDelay(PdMessage *initMessage, PdGraph *graph)
    : DelayReceiver(0, 1, 0, 1, graph) {
...
void DspVariableDelay::processDspWithIndex(int fromIndex, int toIndex) {
    ...
    // calculate delay in samples (vector version of
    // StaticUtils::millisecondsToSamples)
    float samplesPerMillisecond = sampleRate / 1000.0f;
    vDSP_vsmul(dspBufferAtInlet[0], 1, &samplesPerMillisecond, xArray, 1,
               blockSizeInt);

    float zero = 0.0f;
    float one = 1.0f;
    vDSP_vclip(xArray, 1, &zero, &bufferLengthFloat, xArray, 1,
               blockSizeInt); // clip the delay between 0 and the buffer length
    vDSP_vramp(&targetIndexBase, &one, targetIndexBaseArray, 1,
               blockSizeInt); // create targetIndexBaseArray
    vDSP_vsub(
        xArray, 1, targetIndexBaseArray, 1, xArray, 1,
        blockSizeInt); // targetIndexBaseArray - xArray (== targetSampleIndex)
    ...
    // do table lookup (in buffer) using xArray as indicies, with linear
    // interpolation
    vDSP_vlint(buffer, xArray, 1, dspBufferAtOutlet[0], 1, blockSizeInt,
               bufferLength);
#else
    float *inputbuffer = dspBufferAtInlet[0];
    for (int i = 0; i < blockSizeInt; i++, targetIndexBase += 1.0f) {
        float delayInSamples =
            StaticUtils::millisecondsToSamples(inputbuffer[i], sampleRate);
