...
void PdMessage::initWithSARb(unsigned int maxElements, char *initString,
                             PdMessage *arguments, char *buffer,
                             unsigned int bufferLength) {
    resolveString(initString, arguments, 0, buffer,
                  bufferLength); // resolve string
    initWithString(0.0, maxElements, buffer);
}

void PdMessage::initWithString(double ts, unsigned int maxElements,
                               char *initString) {
    timestamp = ts;
...
void PdMessage::resolveString(char *initString, PdMessage *arguments,
    ...
    if (initString == NULL) {
        ...
        strcpy(buffer,
               initString); // NULL arguments returns the original string
    } else {
        ...
        while ((argPos = strstr(initString + initPos, "\\$")) != NULL) {
            ...
            // int argumentIndex = argPos[2] - '0'; (equivalent to below, but
            // below is more clear)
            int argumentIndex = 0;
            switch (argPos[2]) {
            case '0': {
            ...
            if (argumentIndex >= 0 &&
                argumentIndex < numArguments) { // bounds check
                switch (arguments->getType(argumentIndex)) {
                case FLOAT: {
                    numCharsWritten =
                        snprintf(buffer + bufferPos, bufferLength - bufferPos,
                ...
                numCharsWritten =
                    snprintf(buffer + bufferPos, bufferLength - bufferPos, "0");
                bufferPos += numCharsWritten;
...
unsigned int PdMessage::numBytes() { return PdMessage::numBytes(numElements); }
...
int PdMessage::getNumElements() { return numElements; }
...
bool PdMessage::atomIsEqualTo(unsigned int index, MessageAtom *messageAtom) {
    ...
    if (atom->type == messageAtom->type) {
        switch (atom->type) {
        case FLOAT:
            return (atom->constant == messageAtom->constant);
        case SYMBOL:
            return !strcmp(atom->symbol, messageAtom->symbol);
        case BANG:
            return true;
        default:
            return false;
...
void PdMessage::setTimestamp(double timestamp) { this->timestamp = timestamp; }

double PdMessage::getTimestamp() { return timestamp; }

#pragma mark -
#pragma mark initWithTimestampeAnd

void PdMessage::initWithTimestampAndNumElements(double aTimestamp,
                                                unsigned int numElem) {
    memset(this, 0,
           numBytes(numElem)); // clear the entire contents of the message
...
PdMessage *PdMessage::copyToHeap() {
    ...
    memcpy(pdMessage, this, numBytes()); // copy entire structure (but symbol
                                         // pointers must be replaced)
    for (int i = 0; i < numElements; i++) {
        if (isSymbol(i)) {
...
char *PdMessage::toString() {
    ...
    char *finalString; // the final buffer we will pass back after concatenating
                       // all strings - user should free it
    ...
    // chrism: apparently this might fail under MSVC because of snprintf(NULL) -
    // do we care?
    for (int i = 0; i < numElements; i++) {
        lengths[i] = 0;
        switch (getType(i)) {
        ...
        // total length of our string is each atom plus a space, or \0 on the
        // end
        size += lengths[i] + 1;
    }
