...
MessageObject *MessageMakefilename::newObject(PdMessage *initMessage,
                                              PdGraph *graph) {
    return new MessageMakefilename(initMessage, graph);
}

MessageMakefilename::MessageMakefilename(PdMessage *initMessage, PdGraph *graph)
    : MessageObject(1, 1, graph) {
    format = StaticUtils::copyString(
        initMessage->isSymbol(0) ? initMessage->getSymbol(0) : "");
}

MessageMakefilename::~MessageMakefilename() { free(format); }
