...
MessageTable::MessageTable(PdMessage *initMessage, PdGraph *graph)
    : RemoteMessageReceiver(0, 0, graph) {
    if (initMessage->isSymbol(0)) {
        name = StaticUtils::copyString(initMessage->getSymbol(0));
        // by default, the buffer length is 1024. The buffer should never be
        // NULL.
        bufferLength = initMessage->isFloat(1) ? (int)initMessage->getFloat(1)
    ...
    if (newBufferLength > 0) {
        ...
        if (newBufferLength > bufferLength) {
            ...
            memset(buffer + bufferLength, 0,
                   (newBufferLength - bufferLength) * sizeof(float));
        }
        bufferLength = newBufferLength;
        // NOTE(mhroth): this code does not check to see if the returned buffer
        // from realloc is non-NULL. It assumes that realloc is always
        // successful. This is slightly dangerous and could lead to problems.
    }
    return buffer;
}
...
// normalise the contents of the table to the given value. Default to 1.
        ...
        if (sum != 0.0f) {
            ...
            sum = fabsf(sum) /
                  (message->isFloat(1) ? message->getFloat(1) : 1.0f);
