...
MessageObject *DspLowpassFilter::newObject(PdMessage *initMessage,
                                           PdGraph *graph) {
    return new DspLowpassFilter(initMessage, graph);
}

DspLowpassFilter::DspLowpassFilter(PdMessage *initMessage, PdGraph *graph)
    : DspFilter(2, graph) {
    calcFiltCoeff(initMessage->isFloat(0) ? initMessage->getFloat(0)
    ...
    case 0: {
        ...
        case SYMBOL: {
            if (message->isSymbol(0, "clear")) {
                x1 = x2 = dspBufferAtOutlet[0][0] = dspBufferAtOutlet[0][1] =
                    0.0f;
            }
            break;
        }
