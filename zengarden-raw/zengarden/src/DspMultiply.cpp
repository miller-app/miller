...
DspMultiply::DspMultiply(PdMessage *initMessage, PdGraph *graph)
...
void DspMultiply::onInletConnectionUpdate(unsigned int inletIndex) {
    if (incomingDspConnections[0].size() > 0 &&
        ...
        // because onInletConnectionUpdate can only be called at block
        // boundaries, it is guaranteed that no messages will be in the message
        // queue.
...
void DspMultiply::processSignal(DspObject *dspObject, int fromIndex,
                                int toIndex) {
...
void DspMultiply::processScalar(DspObject *dspObject, int fromIndex,
                                int toIndex) {
    DspMultiply *d = reinterpret_cast<DspMultiply *>(dspObject);
