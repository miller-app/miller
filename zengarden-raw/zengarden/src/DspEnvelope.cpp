...
DspEnvelope::DspEnvelope(PdMessage *initMessage, PdGraph *graph)
    : DspObject(0, 1, 1, 0, graph) {
    if (initMessage->isFloat(0)) {
        if (initMessage->isFloat(1)) {
            // if two parameters are provided, set the window size and window
            // interval
    ...
    // NOTE(mhroth): I haven't thought very much if this fix could be better
    // done. The issue is that if the blocksize is large (e.g., larger than the
    // windowInterval), then env~ will never send a message. With more logic, a
    // block size larger than the window size could be accomodated. But I am too
    // lazy to consider this option at the moment. Thus, currently the window
    // size and interval must be at least as large as the block size.
    if (windowSize < graph->getBlockSize()) {
        graph->printErr("env~ window size must be at least as large as the "
                        "block size. %i reset to %i.",
                        windowSize, graph->getBlockSize());
        windowSize = graph->getBlockSize();
    }
    if (windowInterval < graph->getBlockSize()) {
        graph->printErr("env~ window interval must be at least as large as the "
                        "block size. %i reset to %i.",
...
string DspEnvelope::toString() {
    char str[snprintf(NULL, 0, "%s %i %i", getObjectLabel(), windowSize,
                      windowInterval) +
             1];
    snprintf(str, sizeof(str), "%s %i %i", getObjectLabel(), windowSize,
             windowInterval);
    return string(str);
}
...
void DspEnvelope::setWindowInterval(int newInterval) {
    ...
    if (i == 0) {
        ...
        this->windowInterval =
            (newInterval / graph->getBlockSize()) * graph->getBlockSize();
    } else {
        // windowInterval is closer to the larger multiple of blockSize
        this->windowInterval =
            ((newInterval / graph->getBlockSize()) + 1) * graph->getBlockSize();
    }
}
...
void DspEnvelope::initBuffers() {
    ...
    int numBlocksPerWindow = (windowSize % graph->getBlockSize() == 0)
                                 ? (windowSize / graph->getBlockSize())
                                 : (windowSize / graph->getBlockSize()) + 1;
    ...
    for (int i = 0; i < windowSize; i++) {
        ...
        hanningCoefficients[i] =
            0.5f * (1.0f - cosf((2.0f * M_PI * (float)i) / N_1));
        hanningSum += hanningCoefficients[i];
    }
    for (int i = 0; i < windowSize; i++) {
        // normalise the hanning coefficients such that they represent a
        // normalised weighted averaging
        hanningCoefficients[i] /= hanningSum;
    }
}

// windowSize and windowInterval are constrained to be multiples of the block
// size
void DspEnvelope::processSignal(DspObject *dspObject, int fromIndex,
                                int toIndex) {
    DspEnvelope *d = reinterpret_cast<DspEnvelope *>(dspObject);

    // copy the input into the signal buffer
    memcpy(d->signalBuffer + d->numSamplesReceived, d->dspBufferAtInlet[0],
    ...
    if (d->numSamplesReceivedSinceLastInterval == d->windowInterval) {
        ...
        vDSP_vsq(d->signalBuffer, 1, rmsBuffer, 1,
                 d->windowSize); // signalBuffer^2
        vDSP_vmul(rmsBuffer, 1, d->hanningCoefficients, 1, rmsBuffer, 1,
                  d->windowSize);                    // * hanning window
        vDSP_sve(rmsBuffer, 1, &rms, d->windowSize); // sum the result
#else
        for (int i = 0; i < d->windowSize; ++i) {
            rms += d->signalBuffer[i] * d->signalBuffer[i] *
                   d->hanningCoefficients[i];
        }
#endif
        // finish RMS calculation. sqrt is removed as it can be combined with
        // the log operation. result is normalised such that 1 RMS == 100 dB
        rms = 10.0f * log10f(rms) + 100.0f;

        PdMessage *outgoingMessage = PD_MESSAGE_ON_STACK(1);
        // graph will schedule this at the beginning of the next block because
        // the timestamp will be behind the block start timestamp
        outgoingMessage->initWithTimestampAndFloat(0.0,
                                                   (rms < 0.0f) ? 0.0f : rms);
