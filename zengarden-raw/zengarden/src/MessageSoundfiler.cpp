...
MessageObject *MessageSoundfiler::newObject(PdMessage *initMessage,
                                            PdGraph *graph) {
    return new MessageSoundfiler(initMessage, graph);
}

MessageSoundfiler::MessageSoundfiler(PdMessage *initMessage, PdGraph *graph)
...
void MessageSoundfiler::processMessage(int inletIndex, PdMessage *message) {
    if (message->isSymbol(0, "read")) {
        ...
        if ((table = graph->getTable(tabName)) == NULL) {
            graph->printErr("[soundfiler]: table '%s' cannot be found",
                            tabName);
            return;
        }
        SF_INFO sfInfo;
        ...
        if (fullPath == NULL) {
            graph->printErr("[soundfiler]: file '%s' cannot be found.",
                            soundfilePath);
            return;
        }
        ...
        if (sndFile == NULL) {
            graph->printErr("[soundfiler]: file %s cannot be opened.",
                            fullPath);
            free(fullPath);
        ...
        sf_read_float(sndFile, buffer,
                      bufferLength); // read the whole file into memory
        ...
        if (sfInfo.channels > 0) // sanity check
        {
            ...
            float *tableBuffer = shouldResizeTable
                                     ? table->resizeBuffer(samplesPerChannel)
                                     : table->getBuffer(&tableLength);
            if (tableLength > samplesPerChannel) {
                // avoid trying to read more into the table buffer than is
                // available
                tableLength = samplesPerChannel;
            }

            // extract the first channel
            for (int i = 0, j = 0; i < bufferLength;
                 i += sfInfo.channels, j++) {
                tableBuffer[j] = buffer[i];
            }

            // extract the second channel (if it exists and if there is a table
            // to write it to)
            if (sfInfo.channels > 1 &&
                (tabName = message->getSymbol(currentElementIndex++)) != NULL &&
                (table = graph->getTable(tabName)) != NULL) {
                tableLength = samplesPerChannel;
                tableBuffer = shouldResizeTable
                                  ? table->resizeBuffer(samplesPerChannel)
                                  : table->getBuffer(&tableLength);
                if (tableLength > samplesPerChannel) {
                    // avoid trying to read more into the table buffer than is
                ...
                for (int i = 1, j = 0; i < bufferLength;
                     i += sfInfo.channels, j++)
                    tableBuffer[j] = buffer[i];
            }
        }
        ...
        // Not implemented yet
        graph->printErr(
            "[soundfiler]: The 'write' command is not supported yet.");
    }
}
